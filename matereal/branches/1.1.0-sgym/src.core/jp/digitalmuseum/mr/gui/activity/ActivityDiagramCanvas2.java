 ...
 * @author Jun KATO
 */
public class ActivityDiagramCanvas2 extends PCanvas implements
		DisposableComponent {
	...
	private Point2D extensionPoint;

	public ActivityDiagramCanvas2(final ActivityDiagram ad) {
		this.ad = ad;
		synchronized (ad) {
			...
			extensionPoint = new Point2D.Float();
	...
	/**
	 * <b>Cycle removal:</b> constructs an acyclic graph with the provided root node.
	 *
	 * @param rootNode
	...
	private PNodeAbstractImpl constructAcyclicGraph(Node rootNode, int depth) {
		...
		// Cycle removal and layer assignment
		// in: currentNodes
		// out: nodeMap (PNodeAbstractImpl), layers
		Array<Array<PNodeAbstractImpl>> layers = new Array<Array<PNodeAbstractImpl>>();
		Array<Node> currentNodes = new Array<Node>();
		Array<Node> nextNodes = new Array<Node>();
		Array<PNodeAbstractImpl> currentLayer = new Array<PNodeAbstractImpl>();
		Array<PNodeAbstractImpl> nextLayer;
		...
		while (currentNodes.size() > 0) {
			...
			nextLayer = new Array<PNodeAbstractImpl>();

			for (Node node : currentNodes) {
				PNodeAbstractImpl pNode = PNodeFactory.newNodeInstance(node);
				pNode.setDepth(i);
				currentLayer.push(pNode);
				nodeMap.put(node, pNode);

				node.getTransitionsOut(transitions);
				for (Transition transition : transitions) {
					nextNodes.push(transition.getDestination());
				}
				...
				if (node instanceof ControlNode) {
					...
					if (node instanceof Fork) {
						for (Edge edge : edges) {
							nextNodes.push(edge.getDestination());
						}
					} else if (node instanceof Join) {
						for (Edge edge : edges) {
							nextNodes.push(edge.getSource());
						}
					}
				}
				...
				while (it.hasNext()) {
					...
					if (nodeMap.containsKey(nextNode)) {
						PNodeAbstractImpl nextPNode = nodeMap.get(nextNode);
						nextPNode.children.push(node);
						it.remove();
					} else {
						pNode.children.push(nextNode);
					}
				}
			}
	...
	private boolean constructPNode(Node node, PNodeAbstractImpl parentPNode,
		...
		if (nodeMap.containsKey(node)) {
			...
			pNode.getGraphChildren().add(parentPNode);
			pLineLayer.addChild(PNodeFactory.newEdgeInstance(edge, parentPNode, pNode));
			return false;
		} else {
			pNode = PNodeFactory.newNodeInstance(node);
			pNode.setDepth(parentPNode.getDepth() + 1);
			parentPNode.getGraphChildren().add(pNode);
			nodeMap.put(node, pNode);
			pLineLayer.addChild(PNodeFactory.newEdgeInstance(edge, parentPNode, pNode));
			return true;
		}
	...
	private void layoutNodes(PNodeAbstractImpl rootPNode) {
		...
		S: while (!stack.isEmpty()) {
			...
			while (!children.isEmpty()) {
				...
				if (!child.getGraphChildren().isEmpty()) {
					stack.push(child);
					continue S;
				}
	...
	private synchronized void updateEdgePosition(
		...
		startPoint.setLocation(200, 35);
		startPoint = sourcePNode.localToGlobal(startPoint);
		PNodeAbstractImpl destinationPNode = pLineNodeAbstractImpl
				.getDestinationPNode();
		endPoint.setLocation(0, 35);
		endPoint = destinationPNode.localToGlobal(endPoint);
		if (sourcePNode instanceof PDummyNode) {
			extensionPoint.setLocation(0, 35);
			extensionPoint = sourcePNode.localToGlobal(extensionPoint);
			pLineNodeAbstractImpl.setLine(extensionPoint, startPoint, endPoint);
		} else {
			pLineNodeAbstractImpl.setLine(startPoint, endPoint);
		}
	}

				...
				GLJPanel panel = new GLJPanel() {
					...
					public void paintComponent(Graphics g) {
						...
						for (int i = 0; i < captures.length + 1; i++) {
							g2.fillRect(5 + i * 110, 5, 105, 30);
						}

						g2.setComposite(comp);
						g2.setColor(Color.white);
						for (int i = 0; i < captures.length; i++) {
							g2.drawString("Camera " + (i + 1), 10 + i * 110, 30);
						}
						g2.drawString("Calc cam relation", 10 + captures.length * 110, 30);

						g2.setColor(Color.red);
						g2.drawRect(5 + cameraIndex * 110, 5, 104, 29);
						if (hoveredButtonIndex >= 0 &&
								hoveredButtonIndex <= captures.length + 1) {
							g2.setColor(Color.yellow);
							g2.drawRect(6 + hoveredButtonIndex * 110, 6, 102, 27);
						}
					}
				};
